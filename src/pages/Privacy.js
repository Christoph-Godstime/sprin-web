...
const Privacy = () => {
  ...
  const handleNextSlide = () => {
    setActiveSlideIndex((prevIndex) => {
      if (prevIndex === privacy[activeHeadingIndex].slides.length - 1) {
        if (activeHeadingIndex === privacy.length - 1) {
          return prevIndex;
        } else {
  ...
  const handlePrevSlide = () => {
    setActiveSlideIndex((prevIndex) => {
      if (prevIndex === 0 && activeHeadingIndex === 0) {
        ...
        return privacy[activeHeadingIndex - 1].slides.length - 1;
      } else {
        return prevIndex - 1;
      }
    });
                ...
                <div key={index}>
                  <div>
                    ...
                    <div className="max-h-[300px] overflow-y-auto sm:scrollbar sm:scrollbar-w-[6px] sm:scrollbar-thumb-primary sm:scrollbar-track-transparent sm:scrollbar-thumb-rounded-full sm:scrollbar-track-rounded-full">
                      {activeHeadingIndex === index &&
                        privacy.slides.map((slide, slideIndex) => (
                          <div key={slideIndex}>
                            {activeSlideIndex === slideIndex &&
                              slide.sections.map((section, sectionIndex) => (
                                <div
                                  className="pb-[12px] pl-[10px]"
                                  key={sectionIndex}
                                >
                                  <div className="flex">
                                    <div className="w-[15px] flex justify-end mr-[10px]">
                                      <h4 className=" text-[14px] md:text-[16px] font-[400] text-black leading-[21px] md:leading-[28px]">
                                        {section.number}
                                      </h4>
                                    </div>
                                    <div className="w-full">
                                      <h3 className="text-[14px] md:text-[16px] font-[400] text-black leading-[21px] md:leading-[28px]">
                                        {section.title}
                                      </h3>
                                      {section.texts.map((text, textIndex) => (
                                        <ul
                                          className="list-disc mt-[10px] text-black font-[400] text-[14px] md:text-[16px] leading-[21px] md:leading-[28px] -ml-[10px] md:ml-[20px]"
                                          key={textIndex}
                                        >
                                          <li>{text}</li>
                                        </ul>
