...
const CallMedia = () => {
  const { call, support, peer, socket } = useSelector((state) => state);
  ...
  const addCallMessage = useCallback(
    (call, times, disconnect) => {
      if (call.recipient !== userData?._id || disconnect) {
        const msg = {
          sender: call.sender,
    ...
    <div className="call_modal z-40">
      ...
      >
        ...
        <div className="call_menu">
          ...
          {call.recipient === userData?._id && !answer && (
            <>
              {call.video ? (
                <button
